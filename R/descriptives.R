...
hyper_params_df <- function(names, now, short, long, never) {
  now_l <- now |> dplyr::select(all_of(names)) |> mutate(treatment="Now", replication=row_number())
  short_l <- short |> dplyr::select(all_of(names)) |> mutate(treatment="Short", replication=row_number())
  long_l <- long |> dplyr::select(all_of(names)) |> mutate(treatment="Long", replication=row_number())
  never_l <- never |> dplyr::select(all_of(names)) |> mutate(treatment="Never", replication=row_number())
